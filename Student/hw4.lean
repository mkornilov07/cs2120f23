...
-- Here:
def prod_comm_reverse : β × α → α × β := prod_comm
/-! 
...
-- Here:
def prod_assoc_reverse : (A × B) × C → A × (B × C)
| ((a,b),c) => (a,b,c)
/-!
...
-- Here:
def sum_comm_reverse : β ⊕ α → α ⊕ β := sum_comm
